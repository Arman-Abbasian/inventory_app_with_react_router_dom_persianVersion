...
const FilterExits = ({filters,changeHandler,toggleChangeHandler}) => {
    ...
    const [options,setOptions]=useState({productNames:null,consumingFor:null,exitDelivery:null,jobPosition:null,unit:null});
   ...
   if(!options.productNames){
    ...
    if(!options.consumingFor){
        axios.get(`http://localhost:4000/overall?category=consumingFor`)
        .then(res=>{
            const data=res.data;
     ...
     if(!options.exitDelivery){
        axios.get(`http://localhost:4000/overall?category=exitDelivery`)
        .then(res=>{
            const data=res.data;
      ...
      if(!options.jobPosition){
        axios.get(`http://localhost:4000/overall?category=jobPosition`)
        .then(res=>{
            const data=res.data;
     ...
     //make unig options
     if(!options.unit){
        axios.get(`http://localhost:4000/overall?category=unit`)
        .then(res=>{
            const data=res.data;
                ...
                <>
                    ...
                    <datalist id="consumingForr">
                        {options.consumingFor.map(item=>{
                            return <option key={item.id} value={item.consumingFor}>{item.consumingFor}</option>
                        })}
                    </datalist>  
                </> 
                ...
                <>
                    ...
                    <datalist id="exitDeliveryy">
                        {options.exitDelivery.map(item=>{
                            return <option key={item.id} value={item.exitDelivery}>{item.exitDelivery}</option>
                        })}
                    </datalist>  
                </> 
                ...
                <>
                    ...
                    <datalist id="jobPositionn">
                        {options.jobPosition.map(item=>{
                            return <option key={item.id} value={item.jobPosition}>{item.jobPosition}</option>
                        })}
                    </datalist>  
                </> 
                       {/* make unit options */}
                    ...
                    <datalist id="unitt">
                        {options.jobPosition.map(item=>{
                            return <option key={item.id} value={item.unit}>{item.unit}</option>
                        })}
                    </datalist>  
                </> 
