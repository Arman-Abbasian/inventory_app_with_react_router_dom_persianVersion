...
const FilterEnters = ({ filters, changeHandler, toggleChangeHandler }) => {
  ...
  if (!options.enterTransferee) {
    axios
      .get(`http://localhost:4000/overall?category=enterTransferee`)
      .then((res) => {
        const data = res.data;
          ...
          >
            ...
            <>
              <div className="border rounded-sm focus:border-2 flex items-center w-full p-2 gap-2">
                ...
                <input
                  placeholder="نام تامین کننده را جستجو کنید..."
                  list="supplierr"
                  name="supplier"
                  className="w-full bg-transparent outline-none text-primary_cream"
            ...
            <>
              ...
              <datalist id="enterDeliveryy">
                {options.enterDelivery.map((item) => {
                  return (
                    <option key={item.id} value={item.enterDelivery}>
                      {item.enterDelivery}
                    </option>
                  );
                })}
            ...
            <>
              ...
              <datalist id="enterTransfereee">
                {options.enterTransferee.map((item) => {
                  return (
                    <option key={item.id} value={item.enterTransferee}>
            ...
            <div className="w-full">
              <label className="inline-flex relative items-center cursor-pointer">
                ...
                />
                <div
                  className="w-16 h-7 bg-primary_cream peer-focus:outline-none
                        rounded-md peer  peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[3px] after:left-[90px]
                        after:bg-primary_dark_green after:border-gray-300 after:border after:rounded-md after:h-5 after:w-5  after:transition-all  peer-checked:bg-primary_light_green"
                ></div>
                <span className="ml-3 text-base font-bold text-primary_cream">
