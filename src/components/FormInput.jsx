...
const FormInput = ({name ,label,logo}) => {
    ...
    return ( 
        <div className="flex flex-col gap-3 container mx-auto max-w-md p-2">
            <button className="w-full p-2 bg-red-400" onClick={()=>setIsShow(!isShow)}>{isShow ?'hide' : 'show'} {label} input</button>
            <form onSubmit={formik.handleSubmit} className={`${isShow ? 'block':'hidden'}`}>
                <div className="flex flex-col gap-4 justify-center items-center">
                    ...
                    <button disabled={!formik.isValid} className="py-2 px-4 bg-blue-500 rounded-md w-full disabled:bg-opacity-60" type="submit">Add</button>
                </div>
            </form>
        </div>
     );
